...
For files whose language cannot be determined from config files, use truly language-neutral pseudo-code — no function-call syntax, no arrow functions, no framework API:
...
In the Remediation Plan, do NOT name specific testing frameworks or tools (no bats-core, pytest, JUnit, etc.). Describe the harness pattern in plain prose: "Create a stub that intercepts the external call and records its arguments," "Run the script and capture exit code," etc.

If the file extension or shebang reveals a known language (e.g. `.sh` / `#!/usr/bin/env bash`), check whether a reference file exists for it and use that instead. Generic pseudo-code is reserved for languages genuinely outside the model's competence.
